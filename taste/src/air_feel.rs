...
fn a_standing_jump_is_unsteerable_under_a_held_stick() {
    ...
    let mut p = sim::step(start, StepInput { move_dir: Vec3::ZERO, jump: true }, &world, &t);
    for _ in 0..600 {
        if p.grounded {
            break;
        }
        p = sim::step(p, StepInput { move_dir: Vec3::X, jump: false }, &world, &t);
    }
...
fn the_double_jump_fires_airborne_exactly_once_and_resets_on_landing() {
    ...
    let mut p = sim::step(at_rest(&world), jump, &world, &t);
    assert!(!p.grounded && p.air_jumps == t.air_jumps, "the ground jump spends no air jump");
    p = sim::step(p, jump, &world, &t);
    assert!(
    ...
    assert_eq!(p.air_jumps, t.air_jumps, "grounding restores the air jumps");
}
...
fn a_held_direction_air_jump_leaves_the_horizontal_bitwise_untouched() {
    ...
    for stick in [Vec3::Z, Vec3::NEG_X] {
        let jumped = sim::step(p, StepInput { move_dir: stick, jump: true }, &world, &t);
        let steered = sim::step(p, StepInput { move_dir: stick, jump: false }, &world, &t);
        assert_eq!(
        ...
        assert!(
            (jumped.motion.velocity.y - (t.jump_velocity() * t.air_jump_scale - t.ascent_gravity() * SIM_DT)).abs()
                < EPS,
...
fn a_neutral_air_jump_zeroes_the_horizontal_and_resets_straight_up() {
    ...
    let jumped = sim::step(p, StepInput { move_dir: Vec3::ZERO, jump: true }, &world, &t);
    ...
    assert!(
        (jumped.motion.velocity.y - (t.jump_velocity() * t.air_jump_scale - t.ascent_gravity() * SIM_DT)).abs() < EPS,
        "the reset jump still launches at the scaled vertical: {}",
        jumped.motion.velocity.y
    );
    assert_eq!(jumped.air_jumps, t.air_jumps - 1, "the reset jump spends the air jump like any other");
    ...
    let ground_jumped = sim::step(running, StepInput { move_dir: Vec3::ZERO, jump: true }, &world, &t);
    assert!(
