...
//!
//! This is the interaction layer of the revised camera model (designs/movement-camera-design.md, the
//! 2026-06-29 revision): one free-fly god camera, not cycled modes. The scheme is one directional cluster
//! (4-way) plus a vertical pair (raise / lower), time-shared by the persistent
//! [`Target`](crate::model::Target) toggle: in `Look` the cluster flies the [`Camera`] (forward/back
//! along the ground facing, strafe left/right) and the vertical pair raises/lowers it in world Y; in
//! `Move` they grid-step the selection, camera-relative (the world axis nearest the camera facing, the
//! vertical pair stepping the object's world Y). The verbs read wok-platform input (not egui), so the same
//! grammar maps onto a controller later (gilrs is in wok-platform); the chrome's egui shortcuts (Ctrl+S,
//! Esc) stay separate.
//!
//! The mouse, always live for the camera: a held right-drag looks (yaw / pitch), in either target, via
//! the cursor lock ([`update_cursor_grab`]) - the cursor is hidden and pinned for the drag, restored on
//! release. The look reads raw motion, so the lock changes the feel of nothing. Left-click stays object
//! select and drag-and-drop ([`Interaction::gesture`], below), unaffected.
//!
//! Framing is explicit, never automatic (the doc's fix for the auto-frame zoom-jump): [`framing`] reads
//! the Frame key (`f`) after the render residency reconciles (so the selection's bounds are current) and,
//! on a press with a selection, eases the camera to centre on it ([`Camera::frame_to`]); it also advances
//! any in-progress ease each frame. A selection change moves nothing on its own.
//!
//! The mouse for the big jump: [`Interaction::gesture`] resolves the viewport pointer gestures the well
//! raises (`crate::workspace::editor_area`) - a click selects the instance under the cursor, and a drag
//! grabs it and drops it on the surface under the cursor (drag-and-drop, snapped to the grid). The drag
//! grab is the one piece of cross-frame state, held on [`Interaction`]; the keyboard verbs are stateless.
//!
//! Where it runs: the frame loop's interaction seam (`crate::main`), between the chrome's action drain
//! and the draw. [`keyboard`] reads the cluster each frame and either flies the camera (Look) or returns
//! the selection's grid-step (Move); [`update_cursor_grab`] + the right-drag look and [`framing`] run
//! after the render residency reconciles; the mouse [`Interaction::gesture`] resolves a well gesture
//! against the camera, the render residency, and the grab. Each returns actions the frame loop routes
//! through the single writer, gated so a focused text field (keyboard) or a higher egui layer (mouse)
//! takes the input instead.
//!
//! Pure where it can be: the cluster-to-cardinal mapping ([`cluster_step`] / [`camera_relative_step`]),
//! the grid step ([`grid_step`]), and the cursor-lock decision ([`grab_transition`]) are pure and unit
//! tested; the camera math lives in [`crate::camera`]. `keyboard`, `gesture`, and the lock read an
//! [`InputState`] snapshot or a [`Gesture`], so they are testable by building one.
//!
...
/// The Frame key: centre the camera on the selection and ease it there (an explicit verb, never an
/// automatic jump). A one-shot tap, read in [`framing`].
const FRAME: char = 'f';
...
/// cluster and vertical pair either fly the camera (Look) or step the selection (Move). Returns the
/// actions for the frame loop to route through the single writer (the toggle, and - in Move - the
/// selection's transform edits); the camera mutates in place, since it is frame-loop residency, not model
/// state (so the Look fly routes through neither the writer nor an action).
///
/// Look flies the free-fly [`Camera`]: the cluster steps forward/back along the camera's ground facing
/// and strafes left/right, the vertical pair raises/lowers in world Y ([`Camera::fly`]). Move grid-steps
/// the selection one cell per input, camera-relative: the cluster maps to the world grid axis nearest the
/// camera's ground-facing direction ([`camera_relative_step`]), so a step is always grid-aligned, never
/// diagonal; the vertical pair steps the object's world Y. All Move steps snap to the grid (grid-locked
/// this bite; the snap toggles are R3) and route through the single writer, so the edit dirties the scene
...
pub fn keyboard(
    ...
    match model.shell.target() {
        Target::Look => {
            // The cluster and vertical pair fly the camera in place - camera-only, so nothing routes
            // through the writer. The guard skips a no-op fly, so an idle frame never cancels a Frame ease.
            if dx != 0 || dz != 0 || dy != 0 {
                camera.fly(dx, dz, dy);
            }
        }
            ...
            if dx != 0 || dz != 0 || dy != 0 {
                if let (Some(id), Some(loaded)) = (model.shell.selection(), loaded) {
                    if let Some(placement) = loaded.placement(id) {
                        // Camera-relative: snap the cluster to the world grid axis nearest the camera's
                        // facing, so a step is grid-aligned and never diagonal. The vertical pair is the
                        // object's world Y.
                        let (wx, wz) = camera_relative_step(dx, dz, camera.ground_forward());
...
/// The explicit Frame verb and the per-frame ease advance, run after the render residency reconciles so
/// the selection's world bounds are current (designs/movement-camera-design.md "Focus and framing"). On a
/// Frame-key press with a selection, ease the camera to centre on it ([`Camera::frame_to`]); always
/// advance any in-progress ease ([`Camera::advance`]) so the glide shows this frame. Framing is explicit:
/// a selection change moves nothing on its own (no auto-chasing, no auto-zoom on select).
///
/// Focus-gated like [`keyboard`]: a focused text field or a Ctrl/Super chord leaves the Frame key to the
/// chrome. The key is the press edge only (a one-shot, not a repeat). The selection's bounds come from the
/// render residency's [`instance_aabb`](RenderScene::instance_aabb); an instance with no resolvable bounds
/// (or no selection / no scene) simply does not frame, and the ease advance still runs (a no-op at rest).
pub fn framing(
    input: &InputState,
    typing: bool,
    model: &Model,
    ...
    let frame_key = !typing
        && !input.key_held(NamedKey::Control)
        && !input.key_held(NamedKey::Super)
        && input.char_pressed(FRAME);
    if frame_key {
        if let Some(bounds) =
            model.shell.selection().and_then(|id| render_scene.and_then(|scene| scene.instance_aabb(id)))
        {
            camera.frame_to(bounds);
        }
    }
    camera.advance();
}
...
// (designs/sharp-edges.md, the camera-drag cursor-lock trap): engage on a right press over the well rect
// (NOT pointer_free - egui's well click-sense sets is_using_pointer on the press frame, which would miss
...
mod tests {
    ...
    fn camera_relative_step_snaps_to_the_grid_axis_nearest_the_camera_facing() {
        // Facing north (-Z): forward steps north, right east (D -> +X, the A/D sign), back south, left
        // west - the cluster maps onto the world cardinals exactly when the camera faces an axis.
    ...
    #[test]
    fn keyboard_look_flies_the_camera_with_no_model_action() {
        // Look flies the free-fly camera, observed through the public eye: the precise translate math is
        // camera.rs's; here we pin that the Look target routes the cluster to the camera and emits no
        // model action. (The Move-target asserts are the other tests.)
        ...
        assert!(cam.eye().z < before.z, "forward flies the camera north (-Z): {:?}", cam.eye());
        // The vertical pair raises the camera in world Y.
        let flown = cam.eye();
        let _ = keyboard(&input_with_chars(&['e']), false, &model, None, &mut cam);
        assert!(cam.eye().y > flown.y, "raise (E) flies the camera up in world Y");
    }
