...
pub fn keyboard(
    ...
    match model.shell.target() {
        ...
        Target::Move => {
            ...
            // Layout the screen cardinals ARE the world cardinals, exact - the view looks straight
            // down); the vertical pair steps its world Y, camera-independent. All snapped to the grid
            // (grid-locked, no fine nudge) and routed through the single writer, so the edit dirties the
            // scene and Ctrl+S persists it.
            if dx != 0 || dz != 0 || dy != 0 {
...
fn pick(scene: &RenderScene, camera: &LayoutCamera, pos: Vec2, editor_rect: egui::Rect, size: Vec2) -> Option<InstanceId> {
    let pos_in_rect = pos - Vec2::new(editor_rect.min.x, editor_rect.min.y);
    let (origin, dir) = camera.cursor_ray(pos_in_rect, size);
...
mod tests {
    ...
    fn gesture_without_a_scene_only_deselects_on_a_click() {
        ...
        let cam = LayoutCamera::over(Vec3::ZERO);
