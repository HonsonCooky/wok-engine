...
//! (HLD, "content conventions and integrity"), so this picks the minimal layout and owns it as
...
//!
//! Errors are `Box<dyn Error>`: the editor's load path only needs "did it work, and what is the
...
impl ContentPaths {
    pub fn new(root: PathBuf) -> ContentPaths {
        ContentPaths { root }
    }
...
/// Everything the editor loads from disk at open: the authored forms plus the resolved light
...
/// The light-state names present under `lighting/`, sorted, for the content browser's listing. A
/// missing or unreadable directory reads as an empty list (the browser then lists no lighting),
/// never an error: listing content must not fail the open.
pub fn scan_light_names(paths: &ContentPaths) -> Vec<String> {
    let mut names = Vec::new();
    let Ok(entries) = std::fs::read_dir(paths.lighting_dir()) else { return names };
