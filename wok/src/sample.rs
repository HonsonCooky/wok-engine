...
//! prefab table; placement policy is application-side by design). Flat-bottomed prefabs rest by
//! corner rest, so no corner hovers or sinks; round or organic prefabs rest their bounds' bottom on
//! the terrain sample under the placement centre, optionally sunk a few centimetres, because corner
//! rest would perch a curved underside on its highest bounds corner and float it on any slope.
...
const PREFABS: [(&str, Primitive, Vec3, &str, Rest); 4] = [
    ...
    // Uniform on purpose so the boulder reads as a true sphere, and sunk to match its rounder
    // underside (the bottom curvature is tighter than a squashed ellipsoid's, so it needs a touch
    // more sink to sit grounded rather than tangent).
...
/// Spread around the chunk's middle so the spawn camera sees them against the hills. One crate
/// carries a yaw for visual variety; the rest sit square.
...
/// Generate the sample content on disk through the engine's save paths. The only writing this brief
/// does (no editing or save yet).
...
mod tests {
    ...
    fn placements_rest_on_the_surface_per_their_policy() {
        ...
        for placement in &content.chunk.placements {
            ...
            let expected = match rest_policy(placement.prefab.as_str()) {
                ...
                // four bottom corners plus the centre.
                Rest::Corner => {
    ...
    fn one_crate_is_yawed_and_the_marker_stays_axis_aligned() {
        // Authored-data pins: exactly one crate carries a demonstration yaw (visual variety), and
        // the capsule marker stays square - the convention picking and collision will later lean on
        // (a capsule classifies to its conservative box, which must not outgrow the drawn shape).
        let content = build();
        ...
        for placement in &content.chunk.placements {
            if placement.prefab.as_str() == "marker" {
                assert_eq!(placement.transform.rotation, Quat::IDENTITY, "the capsule marker stays axis-aligned");
            }
        }
    }
