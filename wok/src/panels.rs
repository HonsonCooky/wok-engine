...
fn inspector(ui: &mut egui::Ui, model: &EditorModel, actions: &mut Vec<Action>) {
    ...
    if let Some(prefab) = prefab {
        ...
        if has_aabb_grade_solid(prefab, placement) {
            // The OBB parking decision, surfaced where the user acts (yaw is allowed).
            ui.label(
...
/// Does this placement have a solid shape that collides as its conservative box? Round shapes
/// (sphere, true vertical cylinder) classify exactly and spin freely; everything else is the
/// parked-OBB case the inspector warns about.
fn has_aabb_grade_solid(prefab: &Prefab, placement: &Placement) -> bool {
