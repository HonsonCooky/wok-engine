//! The chrome snapshot suite: renders [`super::chrome`] over models built directly (sharp-edges 3) and
//! asserts each against a committed PNG under `tests/snapshots`. Sharing the composition root with the
//! live frame loop is what makes these a real regression guard. Refresh an intended look change with
...
fn chrome_harness(
    ...
    Harness::builder().with_size(size).wgpu().build(move |ctx| {
        ...
        // residency the snapshots have no camera for, so they pin the default Layout home.
        let _ = chrome(ctx, &model, loaded_scene.as_ref(), crate::camera::Mode::Layout);
    })
}
