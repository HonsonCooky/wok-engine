//! Collide-and-slide: move a capsule through static boxes, sliding along whatever it hits.
//!
//! [`collide_and_slide`] takes a desired displacement and walks it out against the static world:
//! sweep to the first impact, advance to just before it, drop the into-surface part of the leftover
//! motion (project it onto the contact plane), and repeat up to a small cap. The capsule ends up
//! flush against the geometry it met, having slid along it rather than stopping dead.
//!
...
use crate::sweep::sweep_capsule_aabbs_inflated;
...
/// Move `capsule` by `displacement` through the static `aabbs`, sliding along contacts.
///
...
pub fn collide_and_slide(
    ...
    for _ in 0..MAX_ITERS {
        ...
        match sweep_capsule_aabbs_inflated(&cap, remaining, aabbs, SKIN) {
            None => {
...
mod tests {
    ...
    fn moving_into_a_wall_at_an_angle_slides_along_it() {
        ...
        let c = player(Vec3::ZERO);
        let wall = Aabb::new(Vec3::new(2.0, 0.0, -50.0), Vec3::new(3.0, 3.0, 50.0));
    ...
    fn sliding_into_a_corner_resolves_against_both_walls() {
        ...
        let c = player(Vec3::ZERO);
        let wall_x = Aabb::new(Vec3::new(2.0, 0.0, -50.0), Vec3::new(3.0, 3.0, 50.0));
        let wall_z = Aabb::new(Vec3::new(-50.0, 0.0, 2.0), Vec3::new(50.0, 3.0, 3.0));
        let d = Vec3::new(5.0, 0.0, 5.0);
    ...
    fn resting_on_a_floor_reads_grounded() {
        ...
        let c = player(Vec3::ZERO);
        let floor = Aabb::new(Vec3::new(-50.0, -1.0, -50.0), Vec3::new(50.0, 0.0, 50.0));
