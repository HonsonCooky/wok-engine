...
# Project: wok-engine
This repo is the wok-engine: a modular monolith Rust workspace for building 3D games. Layers depend downward only:
wok-platform (substrate) -> the wok-* engine crates -> game (a separate downstream project, not in this workspace).

Design canon lives in designs/. Read it before implementing; it is the source of truth for architecture and conventions:
- high-level-design.md: principles, constituent crates, dependency graph, data flow, validation strategy.
